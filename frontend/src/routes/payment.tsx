...
export default function PaymentTab() {
    ...
    /** Fetches all gate entries from the API and excludes system-generated records (block_location = 'SYS') */
    ...
    /**
     * Handles clicking the payment status button on a row.
     * Blocks action if already paid or gated out.
     * For non-laden containers, enforces that gate-in payment must be completed first.
     * Opens the payment modal with the selected record's data pre-filled.
     */
    const handleStatusClick = (record: any) => {
        ...
        // Non-laden containers must have gate-in payment settled before gate-out payment
    ...
    /**
     * Updates the selected payment method and clears any previously selected
     * e-payment provider and reference number to avoid stale values.
     */
    ...
    /**
     * Validates all payment form fields and submits the payment via PATCH request.
     * Enforces: payment method selection, e-payment provider (if applicable),
     * reference number, valid required amount, and that the paid amount meets the minimum.
     * On success, updates the local records state and closes the modal.
     */
    const handleSubmitPayment = async () => {
        ...
        // Payment amount must be at least equal to the required amount
        ...
        try {
            // For e-payment, use the selected provider name as the method; otherwise use the raw method value
    ...
    /** Closes the payment modal and resets all form field state to their empty defaults */
    ...
    const columns = [
        ...
        },
        {
            ...
            // Laden containers are highlighted in violet; empty containers use a neutral style
            render: (value: any) => {
        ...
        },
        {
            ...
            // Shows "Optional" for laden containers since gate-in payment is not required for them
        ...
        },
        {
            ...
            /**
             * Renders a status button with four possible states:
             * - PAID: already settled (green, disabled)
             * - OPTIONAL: laden container with no required payment (teal outline)
             * - LOCKED: gate-in payment not yet completed (amber, disabled)
             * - UNPAID: ready to accept payment (red, clickable)
             */
            render: (_: any, row: any) => {
                const isLaden = row.move_type?.toLowerCase() === 'laden';
                ...
                <div className="modal-backdrop fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                    <div className="modal-content bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
                        ...
                        <div className="px-8 py-6 space-y-5 overflow-y-auto flex-1 min-h-0">
                            ...
                            {/* Required amount input — pre-filled from the record but editable */}
                            <div>
                            ...
                            {/* Reference number input — shown once a payment method is selected; placeholder adapts to method */}
                            {paymentMethod && (
                                <div className="provider-appear">
